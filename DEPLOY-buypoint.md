...
分级服务地址固定为同源相对路径，不提供手动配置入口——同源部署是唯一支持的形态，
「分级」列自动生效，也不存在 CORS 和混合内容问题。若把看板单独部署在别处（比如 Cloudflare Pages），
「分级」列会保持为 `—`。
...
**7 次重试并非 100% 兜底。** 按 32.5% 的单次失败率，7 次全部失败的概率约 0.04%；
但一轮计算要发 ~90 个请求，整轮至少漏网一个的概率约 3%。漏网时那只标的当轮会显示
「分级」列为 `···`（待补，下一次调度自动重试），而不是报错图标 `!` ——
**只有 API Key 缺失/无效/被吊销（Fuyao code 2001/2003）才会显示为真正的错误**，
因为这种情况不会随时间自愈，其余任何失败原因都按"这轮倒霉，下轮会好"处理。

如果你看到某只标的的行情接口手动 `curl` 明明成功，但「分级」列却显示错误，先看是不是
错误图标 `!`（真报错，通常是 Key 问题）还是 `···`（待补，正常现象，等下一次调度或手动
触发 `/api/refresh` 即可）。
